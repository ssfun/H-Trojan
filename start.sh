...
sed -e "1c :$PORT" -e "s/\$AUUID/$AUUID/g" -e "s/\$MYUUID-HASH/$(caddy hash-password --plaintext $AUUID)/g" -e "s/\$VMESS/$VMESS/g" -e "s/\$TROJAN/$TROJAN/g" /conf/Caddyfile >/etc/caddy/Caddyfile
sed -e "s/\$AUUID/$AUUID/g" -e "s/\$VMESS/$VMESS/g" -e "s/\$PASS/$PASS/g" -e "s/\$TROJAN/$TROJAN/g" /conf/config.json >/usr/local/bin/config.json
